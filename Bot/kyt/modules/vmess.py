...
async def create_vmess(event):
	async def create_vmess_(event):
		...
		cmd = f'printf "%s\n" "{user}" "{pw}" "{pw1}" "{exp}" | /usr/bin/Bot/bot-add-vme'
...
async def trial_vmess(event):
	async def trial_vmess_(event):
		...
		time.sleep(1)
		await event.edit("`Wait.. Setting up an Account`")
		cmd = f'printf "%s\n" "{exp}" | /usr/bin/Bot/bot-trial-vme'
		try:
			a = subprocess.check_output(cmd, shell=True).decode("utf-8")
		except:
			await event.respond(f"**User Trial Successfully Created**",buttons = [
[Button.inline(" TRIAL VMESS ","trial-vmess"),
Button.inline(" CREATE VMESS ","create-vmess")],
[Button.inline(" CHECK VMESS ","cek-vmess"),
Button.inline(" DELETE VMESS ","delete-vmess")],
...
async def cek_vmess(event):
	async def cek_vmess_(event):
		cmd = '/usr/bin/Bot/bot-cek-ws'.strip()

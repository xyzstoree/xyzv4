...
async def recov_ssh(event):
	async def recov_ssh_(event):
		...
		async with bot.conversation(chat) as pw:
			...
			cmd = f'printf "%s\n" "{user}" "{pw}" | /usr/bin/Bot/bot-renew-ssh'
		try:
			a = subprocess.check_output(cmd, shell=True).decode("utf-8")
		except:
...
async def limit_ssh(event):
	async def limit_ssh_(event):
		...
		async with bot.conversation(chat) as pw:
			...
			pw = pw.wait_event(events.NewMessage(incoming=True, from_users=sender.id))
			pw = (await pw).raw_text
			cmd = f'printf "%s\n" "{user}" "{pw}" | /usr/bin/Bot/bot-ganti-ip-ssh'
		try:
			a = subprocess.check_output(cmd, shell=True).decode("utf-8")
		except:
			await event.respond(f"`{user}` ** TIDAK DITEMUKAN! **")
...
async def create_ssh(event):
	async def create_ssh_(event):
		...
		time.sleep(1)
		await event.edit("`Wait.. Setting up an Account`")
		cmd = f'printf "%s\n" "{user}" "{pw}" "{pw1}" "{exp}" | /usr/bin/Bot/bot-add-ssh'
...
async def reco_ssh(event):
	async def reco_ssh_(event):
		...
		time.sleep(1)
		await event.edit("`Wait.. Setting up an Account`")
		cmd = f'printf "%s\n" "{user}" | /usr/bin/Bot/bot-user-ssh'
		citsy = f" cat /etc/xray/city"
		city = subprocess.check_output(citsy, shell=True).decode("ascii")
		try:
			subprocess.check_output(cmd,shell=True)
		except:
			await event.respond(f"**User** `{user}` **Successfully Checked**",buttons = [
[Button.inline(" TRIAL SSH ","trial-ssh"),
Button.inline(" CREATE SSH ","create-ssh")],
[Button.inline(" RENEW SSH ","recov-ssh"),
Button.inline(" DETAIL SSH ","reco-ssh")],
...
async def show_ssh(event):
	async def show_ssh_(event):
		cmd = '/usr/bin/Bot/bot-member-ssh'.strip()
...
async def trial_ssh(event):
	async def trial_ssh_(event):
		...
		time.sleep(1)
		await event.edit("`Wait.. Setting up an Account`")
		cmd = f'printf "%s\n" "{exp}" | /usr/bin/trial-ssh'
		try:
			subprocess.check_output(cmd,shell=True)
		except:
			await event.respond("**User Trial Successfully Created**",buttons = [
[Button.inline(" TRIAL SSH ","trial-ssh"),
Button.inline(" CREATE SSH ","create-ssh")],
[Button.inline(" RENEW SSH ","recov-ssh"),
Button.inline(" DETAIL SSH ","reco-ssh")],
...
async def login_ssh(event):
	async def login_ssh_(event):
		cmd = '/usr/bin/Bot/bot-cek-login-ssh'.strip()
		x = subprocess.check_output(cmd, shell=True, stderr=subprocess.STDOUT, universal_newlines=True)
		print(x)
		z = subprocess.check_output(cmd, shell=True).decode("utf-8")
		await event.respond(f"""
